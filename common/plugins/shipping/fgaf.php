...
class fgaf
{
    ...
    //  构造函数 ——分配数据
    public function fgaf($cfg=array())
    {
    ...
    public function calculate($goods_weight, $goods_amount, $goods_number)
    {
        ...
        if ($goods_amount - $this->configure['amount_above'] >= 0) {
            ...
            $shippingFee = $this->configure['shipping_fee'];
        }

        Yii::warning(' fgaf 运费 $shippingFee = '.$shippingFee, __METHOD__);
        return $shippingFee;
    }
    ...
    {
        Yii::warning(' fgaf 计算运费开始  $goods_amount = '.$goods_amount, __METHOD__);

        if ($goods_amount >= $this->configure['amount_above']) {
            $shippingFee = 0;
            $shippingCode = 'free';
        }
        //  不满足满额包邮的条件，默认现付，用户可选择到付，需要修正shipping_code
        else {
            $shippingFee = $this->configure['shipping_fee'];
            if (!empty($prepay)) {
                $shippingCode = 'fgaf';
