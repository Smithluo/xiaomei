...
function submitRefund() {
	var frm = document.forms['formRefund'];
	var reason = frm.elements['reason'].value;
	var msg_content = frm.elements['refund_note'].value;
	var msg = '';
...
function submitReturn() {
	var frm = document.forms['formReturn'];
	var reason = frm.elements['reason'].value;
	var msg_content = frm.elements['refund_note'].value;
    var tracking_number = frm.elements['tracking_number'].value;
    var express_code = frm.elements['express_code'].value;
    var express_name = frm.elements['express_name'].value;
	var msg = '';
    ...
    if (tracking_number.length == 0) {
		msg += no_tracking_number + '\n';
	}
    if (express_code == 0 && express_name.length == 0) {
		msg += no_express_name + '\n';
	}
...
/*******************************************************************************
 * 订单中的商品返回采购车 @param int orderId 订单号
 */
 ...
 * 修改会员银行卡信息
 */
function bankInfoEdit() {
	var frm = document.forms['bankForm'];
	...
	var msg = '';
    var phone_patt=/^((\(\d{3}\))|(\d{3}\-))?1[3-9]\d{9}$/;

	if (real_name.length > 5 || real_name.length < 2 || !(/^[\u4e00-\u9fa5]+$/.test(real_name))) {
		msg += real_name_invlid + '\n';
	}
    if (bank_name == '0') {
		msg += bank_name_invlid + '\n';
	}
    if (!phone_patt.test(phone_number)) {
		msg += phone_number_invlid + '\n';
	}
    if (bank_account.length < 5) {
		msg += bank_account_invlid + '\n';
	}
