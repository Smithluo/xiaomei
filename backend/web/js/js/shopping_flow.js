...
function jsApiCall()
{
    ...
    $.post('/index.php?m=default&c=flow&a=get_wechat_pay_param', param, function(data){
        goto_pay_money(data);
      }, 'json');
}

function goto_pay_money(result){
    if (result.error)
    {
		if(result.redirect){
    ...
    WeixinJSBridge.invoke(
        "getBrandWCPayRequest", result.content,
        function(res){
...
function jsApiCall4Order(){
    var order_id = $('#pay_order_id').val();
    
    $.ajax({
        type: "POST",
        url: "/default/flow/get_pay_for_order_param.html",
        data: {'order_id':order_id},
        dataType:"json",
        success: function(result){
            goto_pay_money(result);
        },
        error:function(){
            alert("error");
        }
...
function Pay4Order()
{
    if (typeof WeixinJSBridge == "undefined"){
        if( document.addEventListener ){
            document.addEventListener("WeixinJSBridgeReady", jsApiCall4Order, false);
        }else if (document.attachEvent){
            document.attachEvent("WeixinJSBridgeReady", jsApiCall4Order); 
            document.attachEvent("onWeixinJSBridgeReady", jsApiCall4Order);
        }
    }else{
        jsApiCall4Order();
    }
