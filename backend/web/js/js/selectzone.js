...
function SelectZone()
{
  ...
  var filename = location.href.substring((location.href.lastIndexOf("/")) + 1, location.href.lastIndexOf("?")) + "?is_ajax=1";
  this.filename  = arguments[4] ? arguments[4] : null;  // 目标 select 对象
  ...
  this.addItem = function(all, act)
  {
    ...
    if (selOpt.length > 0)
    {
      ...
      Ajax.call(this.filename + "&act="+act+"&add_ids=" + jQuery.toJSON(selOpt), args, this.addRemoveItemResponse, "GET", "JSON");
    }
  }
  ...
  this.dropItem = function(all, act)
  {
    ...
    if (arr.length > 0)
    {
      ...
      Ajax.call(this.filename + "&act="+act+"&drop_ids=" + jQuery.toJSON(arr), args, this.addRemoveItemResponse, 'GET', 'JSON');
    }
  }
